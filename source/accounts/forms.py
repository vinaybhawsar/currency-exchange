...
class PictureWidget(forms.widgets.FileInput):
    def render(self, name, value, attrs=None, **kwargs):
        input_html = super().render(name, value.name, attrs=None, **kwargs)
        img_html = mark_safe(f'<br><br><img style="width:100px;" src="{value.url}"/>')
...
class ChangeProfileForm(forms.Form):
    ...
    avatar = forms.ImageField(widget=PictureWidget, label=_('Avatar'), required=False)
    currency = forms.ChoiceField(label=_('Currency'), required=False, choices=CURRENCIES)
...
class TransactionForm(forms.Form):
    to_user = forms.ModelChoiceField(label=_('To'),
                                     queryset=User.objects.all(),
                                     required=True)
    amount = forms.FloatField(label=_('Amount'), min_value=1, required=True)
    currency = forms.ChoiceField(label=_('Currency'), required=True,
                                 disabled=True,
                                 choices=CURRENCIES)
