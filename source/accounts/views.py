...
from .models import Activation, Transaction
...
class ChangeProfileView(LoginRequiredMixin, FormView):
    ...
    def get_initial(self):
        ...
        initial['currency'] = user.currency
        return initial
    ...
    def form_valid(self, form):
        ...
        converted_price = exchange_currency(Money(Decimal(user.wallet_amount),
                                                  currency=user.currency),
                                            form.cleaned_data['currency'])
        user.currency = form.cleaned_data['currency']
        user.wallet_amount = float(converted_price.amount)
        user.save()
...
class TransactionListView(LoginRequiredMixin, ListView):
    template_name = 'accounts/transaction-list.html'
    model = Transaction
    paginate_by = 10

    def get_queryset(self):
        queryset = super(TransactionListView, self).get_queryset()
        queryset = queryset.filter(Q(to_user=self.request.user) | Q(from_user=self.request.user))
...
class TransactionView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        from_amount = form.cleaned_data['amount']
        from_currency = form.cleaned_data['currency']
        converted_price = exchange_currency(Money(Decimal(form.cleaned_data['amount']),
                                                  currency=from_currency),
                                            to_user.currency)
        to_amount = float(converted_price.amount)
        transaction_obj = Transaction(to_user=form.cleaned_data['to_user'],
                                      to_amount=to_amount,
        ...
        self.request.user.wallet_amount -= from_amount
        transaction_obj.to_user.wallet_amount += to_amount
        with transaction.atomic():
            transaction_obj.save()
            self.request.user.save()
            transaction_obj.to_user.save()
        messages.success(self.request, _('Transaction successful.'))
        return redirect('accounts:new-transaction')
